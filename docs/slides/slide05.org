...
# Time-stamp: <2026-05-07 13:46:53 mura>
:REVEAL:
...
- 不連続点まわりでの評価
   #+begin_quote
   \begin{align}
     &\Biggl|\int_{-\delta}^{\delta}P_{r}(y)f(x{-}y)-\frac{f(x{-}0)+f(x{+}0)}{2}dy\Biggr|\\
     &\le
     \Biggl|\int_{-\delta}^{0}P_{r}(y)\{f(x{-}y)-f(x{+}0)\}dy\Biggr|\\
     &\quad+\Biggl|\int_{0}^{\delta}P_{r}(y)\{f(x{-}y)-f(x{-}0)\}dy\Biggr|\\
     &\le
