...
# #+DATE: Time-stamp: <2020-07-03 10:59:15 mura>
:reveal:
...
** Hilbert 空間
   - 完備性の定義
     ...
     #+begin_src latex
       \begin{equation}
	 \lim_{n,m\to\infty}d(u_{n},u_{m})=0.\;\text{(Cauchy列)}
       \end{equation}
     #+end_src
...
** 正規直交系の例
   - $l^{2}$ 空間の自然な系
     ...
     #+begin_src latex
       \begin{align}
	 &u=(u_{k})_{k=1,2,\dotsc}\\
	 &\sum_{k=1}^{\infty}|u_{k}|^{2}<\infty
       \end{align}
     #+end_src
     #+end_quote
     #+reveal: split
