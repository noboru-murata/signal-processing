...
\begin{tikzpicture}[
    ...
    % --- 1段目 (t_1) ---
    \node [block] (sys1) {};
    \draw [->] (sys1.west) ++(-3.0,0) -- (sys1.west) node[midway, above] {$\Delta(t-t_1)$};
    \draw [->] (sys1.east) -- ++(3.0,0) node[midway, above] {$H(t-t_1)$};

    % --- 2段目 (t_2) ---
    % 1つ目のboxから 0.8cm 下に配置（あきすぎを解消）
    \node [block, below=of sys1] (sys2) {};
    \draw [->] (sys2.west) ++(-3.0,0) -- (sys2.west) node[midway, above] {$\Delta(t-t_2)$};
    \draw [->] (sys2.east) -- ++(3.0,0) node[midway, above] {$H(t-t_2)$};
    ...
    \draw [->] (sysn.west) ++(-3.0,0) -- (sysn.west) node[midway, above] {$\Delta(t-t_n)$};
    \draw [->] (sysn.east) -- ++(3.0,0) node[midway, above] {$H(t-t_n)$};
