...
\begin{tikzpicture}[
    ...
    % --- 3. 手書きの解説テキスト（下部） ---
    \node [below left=0.1cm and 0.5cm of BL, align=left] {$t > 0$ のときは \\ $ia$ を囲む積分路};
    ...
    % 【左側の注釈】
    \node [green_circle] (note_L) at (-6, 2) {$\displaystyle\frac{e^{i\omega t}}{\omega - ia}$};
    ...
    % 【右側の注釈】
    \node [green_circle] (note_R) at (6, 2) {$\displaystyle\frac{e^{i\omega t}}{\omega - ia}$};
    ...
    % 【上側の注釈】
    \node [green_circle] (note_T) at (2.5, 4.3) {$\displaystyle\frac{e^{i\omega t}}{\omega - ia}$};
    % 上側は手書き通り「分子の omega」を囲みます
    \node [omega_circle] (wT) at ($(note_T.center)+(0.05, 0.42)$) {};
    \node [text_red, right=0.3cm of note_T, yshift=0.3cm] (red_T) {$\omega$ の虚部 $\to \infty$ \\ $t > 0$};
