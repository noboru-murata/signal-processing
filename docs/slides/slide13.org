...
# #+DATE: Time-stamp: <2020-08-23 22:45:25 mura>
:reveal:
...
** 離散 Fourier 変換
   - 定義
     ...
     #+end_src
     #+end_quote
     - 慣習的に $n$ や $t$ は1ではなく0から始まることに注意する
   #+reveal: split
   - 逆変換の定義
     #+begin_quote
     逆変換は以下で定義される．
     ...
     - 基底は $e^{i\frac{2\pi}{N}t}$ (基本周波数の信号)の高調波になる
...
** 練習問題
   ...
   - 問題2
     #+begin_quote
     "負"の基本周波数 $e^{-i\frac{2\pi}{N}t}$ と標本点 ($t$ が整数の点)で
     同じ値を持つ信号 $e^{i\frac{2\pi}{N}kt}\;(k>0)$ を求めよ．
     #+end_quote
   #+reveal: split
   - 解答例1
     #+begin_quote
...
** COMMENT 注意
   - 確認テスト (講義13の中)
     - 期間: 7/24講義終了-7/30 13:00
     - 時間制限: 1時間 
     - 問 (標本化定理と離散 Fourier 変換の問題)
     - 試験の意味を考えて誠実な対応をすること
